...
const Profile = () => {
  ...
  const isChanged =
    JSON.stringify(currentUser) !== JSON.stringify(inicialState);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (
      currentUser.username !== inicialState.username ||
      currentUser.email !== inicialState.email
    ) {
      if (!token) {
      ...
      changeProfileData(
        ...
        user.set((prev) => {
          return {
            ...
            user: {
              ...prev.user,
              username: currentUser.username ?? prev.user.username,
              email: currentUser.email ?? prev.user.email,
            },
          };
        });
        if (data.status === 200)
  ...
  return (
    <div className=" absolute flex flex-col rounded-xl bg-transparent transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2 w-fit h-fit ">
      <h4 className="block text-xl font-medium text-slate-800">Profile</h4>
      ...
      >
        <div className="mb-1 flex flex-col gap-6">
          ...
          <div className="w-full max-w-sm min-w-[200px]">
            ...
            <input
              ...
              onChange={(e) => handleChange(e, "previousPassword")}
              type="password"
              className="w-full bg-transparent placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded-md px-3 py-2 transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-300 shadow-sm focus:shadow"
        ...
        {informationMessage.status === "success" &&
        informationMessage.message ? (
          <Alert className=" text-green-500">{`${informationMessage.message}!!`}</Alert>
        ) : (
          informationMessage.status === "error" &&
          informationMessage.message && (
            <Alert className=" text-red-500">{`${informationMessage.message}!!`}</Alert>
          )
        )}
        <button
          type="submit"
